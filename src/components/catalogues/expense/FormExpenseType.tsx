...
import { ExpenseTypeModel } from "../models/models";
...
function FormExpenseType(props: any) {
    ...
    const getData = useCallback(() => {
        apiAxios.getAuth(`/catalogue/expense-types/${id}`)
            .then(x => {
    ...
    {
        e.preventDefault();
        let data: ExpenseTypeModel = new ExpenseTypeModel(description,true);
    ...
    const updateExpenseType = (expenseType: ExpenseTypeModel) => {
        apiAxios.patchAuth(`/catalogue/expense-types/${id}`, expenseType)
        .then(x => {
            if(x.status === 200) {
                MySwal.fire({
                    title: 'Updated',
    ...
    const saveExpenseType = (expenseType: ExpenseTypeModel) => {
        apiAxios.postAuth('/catalogue/expense-types', expenseType)
            .then(x => {
                if(x.status === 201) {
                    MySwal.fire({
                        title: "Created",
